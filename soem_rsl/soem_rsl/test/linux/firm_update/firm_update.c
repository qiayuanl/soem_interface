...
int input_bin(char* fname, int* length) {
  ...
  fp = fopen(fname, "rb");
  if (fp == NULL) return 0;
  while (((c = fgetc(fp)) != EOF) && (cc < FWBUFSIZE)) filebuffer[cc++] = (uint8)c;
  *length = cc;
  fclose(fp);
  return 1;
}
...
void boottest(char* ifname, uint16 slave, char* filename) {
  ...
  /* initialise soem_rsl, bind socket to ifname */
  if (ec_init(ifname)) {
...
int main(int argc, char* argv[]) {
  printf("soem_rsl (Simple Open EtherCAT Master)\nFirmware update example\n");
